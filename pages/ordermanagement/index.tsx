...
function Index() {
  ...
  useEffect(() => {
    try {
      axios
        .get(
          `http://localhost:3006/cart/admin/listorder/search?search=searchall&sortBy=${search}`
        )
        .then((res: any) => {
          setOrders(res.data);
        });
  ...
  useEffect(() => {
    try {
      axios
        .get(`http://localhost:3006/cart/admin/listorder?page=${page}`)
        .then((res) => {
          setOrders(res.data);
        });
    } catch (error) {
  ...
  const handleClickCartTotal = () => {
    try {
      axios
        .get(
          `http://localhost:3006/cart/admin/listorder/search?search=${filterCartTotal}&sortBy=cartTotal`
        )
        .then((res: any) => {
          setOrders(res.data);
  ...
  const handleClickUserId = () => {
    try {
      axios
        .get(
          `http://localhost:3006/cart/admin/listorder/search?search=${filterUserId}&sortBy=userId`
        )
        .then((res: any) => {
          setOrders(res.data);
  ...
  const handleClickDate = () => {
    try {
      axios
        .get(
          `http://localhost:3006/cart/admin/listorder/search?search=${filterDate}&sortBy=createdAt`
        )
        .then((res: any) => {
          setOrders(res.data);
  ...
  const handleClickStatus = () => {
    try {
      axios
        .get(
          `http://localhost:3006/cart/admin/listorder/search?search=${filterStatus}&sortBy=status`
        )
        .then((res: any) => {
          setOrders(res.data);
                      ...
                      <div>
                        ...
                        <div className="flex justify-evenly gap-5">
                          <Button
                            ...
                            onClick={() => {
                              try {
                                axios.patch(`http://localhost:3006/cart/admin/listorder/${ordersById.id}`,
                                {
                      ...
                      <button
                        ...
                        onClick={() => {
                          try {
                            axios
                              .delete(
                                `http://localhost:3006/cart/admin/listorder/${ordersById.id}`
                              )
                              .then((res) => {
                                setDeleteModal(!deleteModal);
                                setEditModal(!editModal);
